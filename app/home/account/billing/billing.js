...
angular.module('secxbrl')
    .controller('BillingCtrl', function($scope, $modal, API, apiStatistics) {
        ...
        $scope.getBillingInfo = function() {
            ...
            API.Billing.getBillingInfo({ _method: 'POST', token: $scope.token, $refresh: true })
                .then(
                function(data) {
        ...
        $scope.getSubscriptions = function() {
            ...
            $scope.subscriptions = [];
            $scope.hasPro = false;
            API.Billing.subscriptions({ _method: 'POST', token: $scope.token, $refresh: true })
                .then(
                function(data) {
        ...
        $scope.updateBillingInfo = function() {
            ...
            modal.result
                .then(function(data) {
                    $scope.loadingBillingInfo = true;
                    API.Billing.setBillingInfo({ _method: 'POST', recurlyToken: data.token, token: $scope.token, $refresh: true })
                        .then(
                        function(data) {
                            $scope.billingInfo = data;
                            $scope.loadingBillingInfo = false;
        ...
        $scope.getInvoiceURL = function(invoiceNumber, format) {
            return API.API_URL + '/_queries/public/billing/invoice.jq?_method=POST&invoiceNumber=' + invoiceNumber + '&format=' + format + '&token=' + $scope.token;
        };
    })


    .controller('BillingInfoModalCtrl', function ($scope, $modalInstance, API, billingData, RECURLY_KEY) {
    ...
    .controller('SubscribeModalCtrl', function ($scope, $modalInstance, API, plan) {
        $scope.attempted = false;
        $scope.loading = false;
        $scope.model = { };
        ...
        if (!recurly.configured)
        {
            recurly.configure(API.RECURLY_KEY);
        }
        ...
        $scope.submit = function() {
            ...
            if(!$scope.model.form.$invalid) {
                ...
                API.Billing.createSubscription({ _method: 'POST', plan: $scope.model.data.plan, coupon: $scope.model.data.coupon, token: $scope.token, $refresh: true })
                    .then(
                    function(data) {
    ...
    .controller('UnsubscribeModalCtrl', function ($scope, $modalInstance, API, uuid, plan) {
        $scope.attempted = false;
        $scope.loading = false;
        ...
        $scope.submit = function() {
            $scope.attempted = true;
            $scope.loading = true;
            API.Billing.cancelSubscription({ _method: 'POST', recurlyId: $scope.model.data.uuid, plan: $scope.model.data.plan, token: $scope.token, $refresh: true })
                .then(
                function(data) {
                    $modalInstance.close({ data: data });
                },
