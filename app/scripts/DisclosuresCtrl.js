angular.module('main').controller('DisclosuresCtrl', ['$scope', '$route', '$backend', 'QueriesService', 'years', 'periods',
  function($scope, $route, $backend, QueriesService, years, periods) {
    ...
    $scope.$watch(function() { return angular.toJson($scope.year) + angular.toJson($scope.period); }, 
        function (nv, ov) {
           $scope.data = [];
           $scope.db.forEach(function(item) { if ($scope.year.indexOf(parseInt(item.FiscalYear))>=0 && $scope.period.indexOf(item.FiscalPeriod)>=0) $scope.data.push(item); });
        }
    );
  }
]);
