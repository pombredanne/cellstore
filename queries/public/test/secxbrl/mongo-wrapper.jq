...
import module namespace sec-mw = "http://28.io/modules/xbrl/profiles/sec/mongo-wrapper";
...
let $indexes:=
  ...
  for $coll in keys($sec-mw:ALL-INDEXES)
  for $index in members($sec-mw:ALL-INDEXES($coll))
  return local:check-index($conn, $coll, $index)
return 
  if (count($indexes) eq 21)
  then "OK"
  else error(QName("sec-mw:TEST-FAIL"), "Expected 21 indexes, got " || count($indexes))
