...
declare %rest:case-insensitive variable  $cursor       as string?           external;
declare %rest:case-insensitive variable  $limit        as positiveInteger?  external := 10;
...
let $info := recurly-api:list-invoices($user, $cursor, $limit)
return
    switch ($format)
    case "xml" return {
