...
declare function local:compileFilingStats($archive) {
    let $statementRegex := "-\\s*statement\\s*-"
    let $faceFinancials :=  components:components-for-archives($archive)[matches($$.Label,$statementRegex,"i")]  
    ...
    return {
        ...
        qtyFaceFinancials: count($faceFinancials)
    }
};
