...
as empty-sequence()
{
    ...
    variable $user := user:get-existing-by-id($user-id);
    variable $assignments := flatten($user.assignments);

    if (empty($fromDateTime))
    then 
        (for $assignment at $idx in $assignments
        where $assignment.role eq $role-id
        return delete json $user.assignments($idx));
    else 
    {
        (for $assignment at $idx in $assignments
        where $assignment.role eq $role-id and $assignment.from-dateTime ge $fromDateTime
        return delete json $assignments($idx));
